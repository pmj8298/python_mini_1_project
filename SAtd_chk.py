...
class SAtdMainWindow(QMainWindow):
    ...
    def atdCheckClick(self):
        ...
        try:
            ...
            if result:
                ...
                if check_result:
                    ...
                    update_query = """UPDATE atd
                                    SET atd_time = CAST(SYSTIMESTAMP AT TIME ZONE 'UTC' AS TIMESTAMP WITH LOCAL TIME ZONE),
                                        atd_date = TRUNC(SYSDATE)
            ...
            error_msg = f"데이터베이스 오류 발생: {str(e)}"
            print(error_msg)  # 터미널에 오류 메시지 출력
            QMessageBox.critical(self, "데이터베이스 오류", error_msg)
